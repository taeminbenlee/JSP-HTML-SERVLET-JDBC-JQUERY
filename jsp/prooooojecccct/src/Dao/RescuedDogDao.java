...
// TODO 작성 
	public boolean writeRescued(RescuedDogDto dto) {
		String sql = " INSERT INTO RESCUEDDOG (SEQ, WDATE, VCOUNT, CONDITION, TITLE, MYCONTENT, FILENAME, NEWFILENAME, DEL) "
		...
		try {
			...
			psmt.setString(4, dto.getNewfilename());
...
// TODO 테이블 출력용 List
	public List<RescuedDogDto> RescuedList(String choice, String search, int page) {
		String sql = " SELECT SEQ, WDATE, VCOUNT, CONDITION, TITLE, MYCONTENT, FILENAME, NEWFILENAME, DEL " 
				   + " FROM ";
												 // 입양 진행중이거나 완료된 것은 뒤에 글로 내린다. 글작성 순으로 올린다
		sql += "         (SELECT ROW_NUMBER()OVER(ORDER BY CONDITION ASC, SEQ DESC) AS RNUM, "
				+ "              SEQ, WDATE, VCOUNT, CONDITION, TITLE, MYCONTENT, FILENAME, NEWFILENAME, DEL "
				+ "         FROM RESCUEDDOG " 
				+ "         WHERE DEL = 0 ";
...
// TODO 작성된 글의 총 수 구하기
	public int getAllList(String choice, String search) {
		String sql = " SELECT COUNT (*) " + " FROM RESCUEDDOG " + " WHERE DEL = 0 ";
...
// TODO 수정
	public boolean updateRescued(RescuedDogDto dto, int seq) {
		String sql = " UPDATE RESCUEDDOG " 
				   + " SET WDATE=SYSDATE, TITLE=?, MYCONTENT=?, FILENAME=?, NEWFILENAME=?"
			       + " WHERE SEQ=? ";
...
//TODO 입양진행중
	public void conditionChange1(int seq, String title) {
		...
		Connection conn = null;
		PreparedStatement psmt = null;
		String changeTitle = "[입양진행중] " + title;
		System.out.println("변경 전 이름 : " + title);
		System.out.println("변경 후 이름 : " + changeTitle);
