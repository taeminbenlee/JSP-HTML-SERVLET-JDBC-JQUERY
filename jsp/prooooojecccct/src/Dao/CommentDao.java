...
public class CommentDao {
	...
	public boolean writeComment(CommentDto dto) {
		String sql = " INSERT INTO MYCOMMENT (MYID, CONTENT, REF, DEL, SEQ) "
	...
	// TODO 테이블 출력용 List (부모글의 SEQ와 같은 REF값을 가진것만 출력)
		public List<CommentDto> CommentList(int page, int seq) {
			String sql = " SELECT MYID, CONTENT, REF, DEL, SEQ " 
					   + " FROM "
			        + "         (SELECT ROW_NUMBER()OVER(ORDER BY SEQ) AS RNUM, "
					+ "              MYID, CONTENT, REF, DEL, SEQ "
					+ "         FROM MYCOMMENT " 
					+ "         WHERE DEL = 0 AND REF=? "
                    + "         ORDER BY SEQ) "
			        + " WHERE RNUM >= ? AND RNUM <= ? ";
		...
		//TODO 댓글 수정
		public boolean updateComment(String content, int seq) {
